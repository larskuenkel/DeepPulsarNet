...
for file in created_files[:]:
    ...
    down -= np.median(down)
    down /= np.max(down)
    peaks, prop = scipy.signal.find_peaks(
        down, height=0.3, distance=approx_dist)

    if dm_shift:
        dm_delays = fil.header.getDMdelays(dm)
        middle_delay = dm_delays[len(dm_delays) // 2]
        peaks += middle_delay
        peaks = peaks[peaks < len(down)]
    ...
    try:
        ...
        acf_peaks, _ = scipy.signal.find_peaks(
            acf[middle - 5: middle + 5000], height=height)
        # print(acf_peaks)
